...
#define DISPLAY_HEX(n)\
	...
	{\
		std::cout << REP_HEX(TARGET_BYTE(n, byte) >> 4);/*Zeroes the leftmost nibble, replacing the rightmost*/\
		std::cout << REP_HEX((TARGET_BYTE(n, byte) << 4) >> 4) << ' ';/*Zeroes the leftmost nibble to capture the rightmost, space for console formatting*/\
	}
//ideally nullptr or an unitialized pointer would be passed in.
#define HEX_ARRAY(n, ptrArr) {\
	ptrArr = new char[sizeof(n)*2];\
	for (int byte = 0; byte < sizeof(n); byte++)\
		ptrArr[byte * 2] = TARGET_BYTE(n, byte) >> 4;/*Zeroes the leftmost nibble, replacing the rightmost*/\
		ptrArr[byte * 2 + 1] = (TARGET_BYTE(n, byte)/*Zeroes the leftmost nibble to capture the rightmost*/ << 4) >> 4;\
	}\
}
